...
class Visualizer:
	...
	**Args:**
	...
	def _update_bounding_boxes(self, animation_frame=None):
		...
		mat.line_width = 4 * self.window.scaling

		track_mat = rendering.Material()
		track_mat.shader = "unlitLine"
		track_mat.line_width = 10 * self.window.scaling
		...
		if self._consolidate_bounding_boxes:
			...
			selected_frames = [idx for idx,bbox_data in enumerate(self._objects.bounding_box_data) 
			...
			if len(boxes) > 0:
				...
				#pred_lines.colors = o3d.utility.Vector3dVector([[0.875, 0.875, 0.875] for _ in range(len(pred_lines.lines))])
				#self._3d.scene.add_geometry(preds_name, pred_lines, mat)
				points, indices, colors, bbox_colors = [],[],[], []
				if np.array(tracks[-1]).shape[0]>=2:
						...
						if (track[-1]!=[-1,-1,-1]).all() and len(idx)>2:
							...
							for ii in range(len(curr_track)):
								if ii==len(curr_track)-1: pass #Ensure we do not connect when starting the new track
									...
									label = Colors[i] if i<len(Colors) else Colors[i-len(Colors)]
									colors.append((label[0], label[1], label[2]))
								tot_idx+=1
							label = Colors[i] if i<len(Colors) else Colors[i-len(Colors)]
							bbox_colors.append((label[0], label[1], label[2]))
					...
					#colorize all selected bboxes in frames
					for last_frame_tracks in tracks:
						for i,track in enumerate(last_frame_tracks[-1]):
							idx = np.where((track[:,0] != -1) & (track[:,1] != -1) & (track[:,2] != -1))[0]
							if (track[-1]!=[-1,-1,-1]).all() and len(idx)>2:
								label = Colors[i] if i<len(Colors) else Colors[i-len(Colors)]
								bbox_colors.append((label[0], label[1], label[2]))

				lines = BoundingBox3D.create_lines(boxes, bbox_colors)
